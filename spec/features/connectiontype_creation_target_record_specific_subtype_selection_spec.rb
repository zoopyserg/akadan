    ...
    context 'specific_subtype' do
      context 'other peoples record types' do
        ...
        it 'should not include someone elses private type' do
          expect(page.all('select#connection_type_target_record_subtype_id option').map(&:text)).not_to include('Someone Elses Private Type')
        end

        it 'should include someone elses public type' do
          expect(page.all('select#connection_type_target_record_subtype_id option').map(&:text)).to include('Someone Elses Public Type')
        end
      end
