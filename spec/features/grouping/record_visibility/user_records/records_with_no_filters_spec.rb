...
RSpec.feature "RecordsIndices", type: :feature do
  ...
  context 'not signed in' do
    context 'user 1 path' do
      ...
      it { expect(current_path).to eq user_path(user1) }
      it { expect(page).to have_content 'Main Record 1' }
    ...
    context 'user 2 path' do
      ...
      it { expect(current_path).to eq user_path(user2) }
      it { expect(page).to have_no_content 'Main Record 1' }
      it { expect(page).to have_content 'Main Record 2' }
