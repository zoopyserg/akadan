...
RSpec.feature "RecordsIndices", type: :feature do
  ...
  before { uncheck "group_record_a_ids_#{record2.id}" }
  before { click_on 'Save Everything' }

  it { expect(current_path).to eq edit_group_path(group) }
  it { expect(page).to have_no_text "Public Record 2" }
  it { expect(page).to have_text "Public Record 3" }
end
