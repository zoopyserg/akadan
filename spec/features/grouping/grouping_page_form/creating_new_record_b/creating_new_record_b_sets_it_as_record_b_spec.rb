...
RSpec.feature "RecordsIndices", type: :feature do
  ...
  it do
    fill_in 'group_new_record_b_name', with: 'Some Magic Record'
    choose_something('The Magic Record Type', '.new_record_b_record_type_section')
    click_on 'Create Record B'
  ...
  it do
    ...
    expect(Connection.all.collect{|x| x.record_b.name}.uniq).to eq ['Some Magic Record']
  end
end
