...
RSpec.feature "User Show Records Show Button", :records_index, type: :feature do
  ...
  let!(:record_show_url) { record_path(record) }
  ...
  context 'public someone elses record' do
    ...
    context 'not signed in' do
      describe 'a button' do
        ...
        it 'should have no button' do
          expect(page).to have_css "[href='#{record_show_url}']"
        end
      end

      describe 'a path authorization' do
        before { visit record_show_url }

        it 'should have no button' do
          expect(current_path).to eq record_show_url
        end
      end
    end
    ...
    context 'signed in user 1' do
      ...
      it 'should let me create subrecords' do
        find(:css, ".record_show_icon_button[href='#{record_show_url}']").click
  ...
  context 'private my record' do
    ...
    context 'not signed in' do
      describe 'a button' do
        ...
        it 'should have no button' do
          expect(page).to have_no_css "[href='#{record_show_url}']"
        end
      end
