...
RSpec.feature "People Index Send Message", type: :feature do
  let!(:user1) do
  ...
  context 'not signed in' do
    ...
    context 'visit user1 page' do
      ...
      it { expect(page).to have_no_css "[href='#{person_conversation_starts_path(user1.id)}']" }
    end

    context 'visit stranger page' do
      before { visit user_path(stranger) }

      it { expect(page).to have_no_css "[href='#{person_conversation_starts_path(stranger.id)}']" }
    end

    context 'visit stranger_protected page' do
      before { visit user_path(stranger_protected) }

      it { expect(page).to have_no_css "[href='#{person_conversation_starts_path(stranger_protected.id)}']" }
    end

    context 'visit friend page' do
      before { visit user_path(friend) }

      it { expect(page).to have_no_css "[href='#{person_conversation_starts_path(friend.id)}']" }
    end

    context 'visit friend_private page' do
      before { visit user_path(friend_private) }

      it { expect(page).to have_no_css "[href='#{person_conversation_starts_path(friend_private.id)}']" }
    end

    context 'visit friend_request_sent_to_him page' do
      before { visit user_path(friend_request_sent_to_him) }

      it { expect(page).to have_no_css "[href='#{person_conversation_starts_path(friend_request_sent_to_him.id)}']" }
    end

    context 'visit friend_request_sent_to_him_private page' do
      before { visit user_path(friend_request_sent_to_him_private) }

      it { expect(page).to have_no_css "[href='#{person_conversation_starts_path(friend_request_sent_to_him_private.id)}']" }
    end

    context 'visit friend_request_sent_to_me page' do
      before { visit user_path(friend_request_sent_to_me) }

      it { expect(page).to have_no_css "[href='#{person_conversation_starts_path(friend_request_sent_to_me.id)}']" }
    end

    context 'visit friend_request_sent_to_me_private page' do
      before { visit user_path(friend_request_sent_to_me_private) }

      it { expect(page).to have_no_css "[href='#{person_conversation_starts_path(friend_request_sent_to_me_private.id)}']" }
    end

    context 'visit blacklisted page' do
      before { visit user_path(blacklisted) }

      it { expect(page).to have_no_css "[href='#{person_conversation_starts_path(blacklisted.id)}']" }
    end

    context 'visit blacklisted_me page' do
      before { visit user_path(blacklisted_me) }

      it { expect(page).to have_no_css "[href='#{person_conversation_starts_path(blacklisted_me.id)}']" }
    end
  ...
  context 'signed in' do
    context 'I sign in' do
      ...
      context 'visit stranger page' do
        ...
        it { expect(page).to have_css "[href='#{person_conversation_starts_path(stranger.id)}']" }
      end
      ...
      context 'visit friend page' do
        ...
        it { expect(page).to have_css "[href='#{person_conversation_starts_path(friend.id)}']" }
      end

      context 'visit friend_private page' do
        before { visit user_path(friend_private) }

        it { expect(page).to have_css "[href='#{person_conversation_starts_path(friend_private.id)}']" }
      end

      context 'visit friend_request_sent_to_him page' do
        before { visit user_path(friend_request_sent_to_him) }

        it { expect(page).to have_css "[href='#{person_conversation_starts_path(friend_request_sent_to_him.id)}']" }
      end
      ...
      context 'visit friend_request_sent_to_me page' do
        ...
        it { expect(page).to have_css "[href='#{person_conversation_starts_path(friend_request_sent_to_me.id)}']" }
      end
      ...
      describe 'sending a message' do
        ...
        it { expect{ find("[href='#{person_conversation_starts_path(stranger.id)}']").click }.to change{ Conversation.count }.by(1) }
        it { expect{ find("[href='#{person_conversation_starts_path(stranger.id)}']").click }.to change{ Participation.where(user: user1).count }.by(1) }
        it { expect{ find("[href='#{person_conversation_starts_path(stranger.id)}']").click }.to change{ Participation.where(user: stranger).count }.by(1) }
      end
    end
    ...
    context 'stranger@gmail.com signs in' do
      ...
      context 'visit user1 page' do
        ...
        it { expect(page).to have_css "[href='#{person_conversation_starts_path(user1.id)}']" }
      end
      ...
      context 'visit blacklisted page' do
        ...
        it { expect(page).to have_css "[href='#{person_conversation_starts_path(blacklisted.id)}']" }
      end

      context 'visit blacklisted_me page' do
        before { visit user_path(blacklisted_me) }

        it { expect(page).to have_css "[href='#{person_conversation_starts_path(blacklisted_me.id)}']" }
      end
