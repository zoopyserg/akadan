...
RSpec.feature "Decising which records to show", :records_index, type: :feature do
  ...
  context 'showing only solved records of specific type' do
    ...
    describe 'raw URL visit' do
      context 'record type 1' do
        context 'not signed in' do
          before { visit records_path(columns: [{ filter_solved_status: 'solved', collapsed: false, record_type_id: record_type1.id, only_separate_projects: false, only_direct_children: false, page: 1 }]) }
        ...
        context 'signed in' do
          before do
            login_as user2
            visit records_path(columns: [{ filter_solved_status: 'solved', collapsed: false, record_type_id: record_type1.id, only_separate_projects: false, only_direct_children: false, page: 1 }])
          end
    ...
    # button is tested incolumns folder
  end
end
