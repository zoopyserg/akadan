    ...
    describe 'raw URL visit' do
      context 'record type 1' do
        context 'not signed in' do
          before {  visit records_path(columns: [{ filter_solved_status: 'unsolved', collapsed: false, record_type_id: record_type1.id, only_separate_projects: false, only_direct_children: false, page: 1 }]) }
        ...
        context 'signed in' do
          before do
            login_as user2
            visit records_path(columns: [{ filter_solved_status: 'unsolved', collapsed: false, record_type_id: record_type1.id, only_separate_projects: false, only_direct_children: false, page: 1 }])
          end
