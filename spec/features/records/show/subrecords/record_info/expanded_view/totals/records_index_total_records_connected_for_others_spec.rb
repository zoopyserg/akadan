...
RSpec.feature "Record Created By", :records_index, type: :feature do
  ...
  let!(:connection6) { create :connection, record_a: main_record1, record_b: public_record, user: user1 }

  let!(:connection7) { create :connection, record_a: main_record1, record_b: private_record6, user: user1 }
  let!(:connection8) { create :connection, record_a: main_record1, record_b: private_record7, user: user1 }
  ...
  context 'signed in' do
    ...
    context 'public' do
      ...
      it { expect(page).to have_content 'Other people\'s records connected: 3' }
    end
