...
// |---(2 destructive) -- solved with (Solution 2)

int setup_all_solved_tree_record_ids_one_destructive_one_non_destructive_subrecord_both_solved(void) {
    common_cleanup();
    ...
    // insert a regular non destructive connection type
    ...
    char* connectionTypeValues2[] = {"Is Non Destructive Related To...", userId, "TRUE", "FALSE", "FALSE", "record", recordTypeId, recordTypeId, "NULL", recordTypeId, "TRUE", "NOW()", "NOW()", NULL};
    ...
    // insert a regular destructive connection type
    char* connectionTypeColumns3[] = {"name", "user_id", "is_public", "directional", "destructive", "target_type", "target_record_type_id", "target_record_subtype_id", "target_hierarchy", "closest_parent_type_id", "one_to_many", "created_at", "updated_at", NULL};
    char* connectionTypeValues3[] = {"Is Destructive Related To...", userId, "TRUE", "FALSE", "TRUE", "record", recordTypeId, recordTypeId, "NULL", recordTypeId, "TRUE", "NOW()", "NOW()", NULL};
    char* connectionTypeId3 = common_insert("connection_types", connectionTypeColumns3, connectionTypeValues3);
    if (!connectionTypeId3) {
        fprintf(stderr, "Setup failed: Unable to insert connection type.\n");
        free(userId); // Clean up the userId string
        free(recordTypeId);
        free(recordId1);
    ...
    char* connectionValues4[] = {"Test Connection 4", userId, "TRUE", rootRecordId, recordId2, connectionTypeId3, "NOW()", "NOW()", NULL};
...
void test_all_solved_tree_record_ids_one_destructive_one_non_destructive_subrecord_both_solved() {
    setup_all_solved_tree_record_ids_one_destructive_one_non_destructive_subrecord_both_solved();
    ...
    sprintf(expectedOutput1, "%s: 1 : 0 : 15.000000 : 100.000000 : %s,%s", rootRecordId, recordId1, recordId2); // philosophical question: if some some sub records are non-destructive, they don't affect the solution at all? Like no matter if they are solved or not, only the destructive ones affect the percentage? For now no. We'll figure out later
    sprintf(expectedOutput2, "%s: 1 : 0 : 15.000000 : 100.000000 : %s,%s", recordId1, solutionId1, rootRecordId);
    sprintf(expectedOutput3, "%s: 1 : 0 : 15.000000 : 100.000000 : %s,%s", recordId2, solutionId2, rootRecordId);
    sprintf(expectedOutput4, "%s: 0 : 1 : 15.000000 : 0.000000 : %s,%s", solutionId1, recordId1, rootRecordId);
    sprintf(expectedOutput5, "%s: 0 : 1 : 15.000000 : 0.000000 : %s,%s", solutionId2, recordId2, rootRecordId);
