...
int setup_all_solved_tree_record_ids_rank_case_one(void) {
    ...
    char* rootRecordColumns1[] = {"user_id", "name", "is_public", "record_type_id", "created_at", "updated_at", NULL};
    char* rootRecordValues1[] = {userId, "Root Record 1", "TRUE", recordTypeId, "NOW()", "NOW()", NULL};
    rootRecordId1 = common_insert("records", rootRecordColumns1, rootRecordValues1);
    if (!rootRecordId1) {
    ...
    char* dotColumns1[] = {"user_id", "record_id", "project_id", "duration", "description", "created_at", "updated_at", NULL};
    char* dotValues1[] = {userId, rootRecordId1, rootRecordId1, "60", "Root Record 1 dot", "NOW()", "NOW()", NULL};
    char* dotId1 = common_insert("dots", dotColumns1, dotValues1);
    if (!dotId1) {
    ...
    char* rootRecordColumns2[] = {"user_id", "name", "is_public", "record_type_id", "created_at", "updated_at", NULL};
    char* rootRecordValues2[] = {userId, "Root Record 2", "TRUE", recordTypeId, "NOW()", "NOW()", NULL};
    rootRecordId2 = common_insert("records", rootRecordColumns2, rootRecordValues2);
    if (!rootRecordId2) {
        fprintf(stderr, "Setup failed: Unable to insert root record.\n");
        return -1;
    }
    ...
    // Dot 1
    char* dotColumns2[] = {"user_id", "record_id", "project_id", "duration", "description", "created_at", "updated_at", NULL};
    char* dotValues2[] = {userId, rootRecordId2, rootRecordId2, "60", "Root Record 2 dot 1", "NOW()", "NOW()", NULL};
    char* dotId2 = common_insert("dots", dotColumns2, dotValues2);
    if (!dotId2) {
        fprintf(stderr, "Setup failed: Unable to insert dot.\n");
        return -1;
    }
    ...
    char* dotId3 = common_insert("dots", dotColumns2, dotValues3);
    if (!dotId3) {
        fprintf(stderr, "Setup failed: Unable to insert dot.\n");
        return -1;
    }
